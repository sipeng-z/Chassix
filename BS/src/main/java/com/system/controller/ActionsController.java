...
@RestController
public class ActionsController  extends GenericController<ActionsInput,ActionsOutput> {
    ...
    protected void OnActionExecuting() {
        actionsService.SetWorkContext(this.WorkContext);
    }

    @Override
    protected AbstractService<ActionsInput,ActionsOutput> getService() {
    ...
    public ResponseResult formPost(String id, @RequestBody ActionsInput input) {
    ...
    public ResponseResult pagerList(HttpServletRequest request) {
        try{
            return ResponseResult.success(new PageInfo<>(actionsService.list(true,new PageData(request))));
        }catch (Exception e){
