...
//2. path list already , loop to get contents for each path
            ...
            String pathF = "D:\\SingleTest\\33~1A0204625@488504.meas9311";
                List<String>  fileContent =   this.txt2String(new File(pathF));
...
//4.loop for HKRXSUB content
                for(int i=0;i<HKRXSUB.length;i++ ){
                    ...
                    if(HKRXVAL>=-1.1000&&HKRXVAL<=-0.9000){ // when x val equals ,put y val;
                        ...
                        fullInfo.putIfAbsent(hkvals,new File(pathF));                   resultMap.putIfAbsent(HKRXVAL,Double.parseDouble(HKRYSUB[i]));
                    }
