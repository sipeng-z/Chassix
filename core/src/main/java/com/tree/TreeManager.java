 ...
 * 描述：获得树形的服务实现
 */
public abstract class TreeManager<T extends BaseTreeObj<T, ID>, ID extends Serializable>
	...
	public List<T> getChildList(List<T> list, T t) {
		...
		while (it.hasNext()) {
			 ...
			 * 判断集合的父ID是否等于上一级的id
			 */
			if (((BaseTreeObj<T, ID>) child).getParentid().equals(((BaseTreeObj<T, ID>) t).getPkId())) {
